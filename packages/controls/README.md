# @stratum-fx/controls · 0.1.0

Dependency-free node editor, parameter editor and timeline custom elements.
...
```sh
npm install /path/to/stratum-fx-controls-0.1.0.tgz
```

Import the named exports from `@stratum-fx/controls`. Browser applications may import `dist/index.js` directly from their own static server. Controls and rendering require the DOM; geometry and core also execute under Node 22.13+.

For a classic script, load `dist/standalone.js` and await `globalThis.StratumControlsReady`; the resolved object is also exposed as `globalThis.StratumControls`. Do not assume an asynchronous bundle is initialized until that promise resolves.
