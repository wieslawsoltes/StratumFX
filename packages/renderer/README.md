# @stratum-fx/renderer · 0.1.0

WebGPU PBR viewport, WebGL2 fallback, CPU preview, orbit camera and BVH picking.
...
```sh
npm install /path/to/stratum-fx-renderer-0.1.0.tgz
```

Import the named exports from `@stratum-fx/renderer`. Browser applications may import `dist/index.js` directly from their own static server. Controls and rendering require the DOM; geometry and core also execute under Node 22.13+.

For a classic script, load `dist/standalone.js` and await `globalThis.StratumRendererReady`; the resolved object is also exposed as `globalThis.StratumRenderer`. Do not assume an asynchronous bundle is initialized until that promise resolves.
