# @stratum-fx/geometry · 0.1.0

Typed-array meshes, procedural geometry, interchange and bounded simulation solvers.
...
```sh
npm install /path/to/stratum-fx-geometry-0.1.0.tgz
```

Import the named exports from `@stratum-fx/geometry`. Browser applications may import `dist/index.js` directly from their own static server. Controls and rendering require the DOM; geometry and core also execute under Node 22.13+.

For a classic script, load `dist/standalone.js` and await `globalThis.StratumGeometryReady`; the resolved object is also exposed as `globalThis.StratumGeometry`. Do not assume an asynchronous bundle is initialized until that promise resolves.
